...
1. **씬 구성**: 각 씬의 목적, 타입, 예상 분량 + **육하원칙(5W1H)**
   ...
   - **씬 간 인과 체인 (매우 중요)**:
     - **triggered_by**: 이 씬이 시작되는 원인. 직전 씬의 어떤 결과가 이 씬을 일으켰는가?
     - **leads_to**: 이 씬의 결과가 다음 씬에 어떤 영향을 주는가?
     - 첫 씬은 triggered_by가 이전 화 끝 상황. 마지막 씬은 leads_to가 다음 화 떡밥.
     - ❌ 씬 3개가 독립적으로 나열 → 독자가 "왜 갑자기?" 느낌
     - ✅ 씬 A의 결론 → 씬 B의 원인 → 씬 C의 원인 (연쇄)
...
13. **대사 턴-테이킹 (scene.dialogue_turns) — 2명 이상 등장 씬 필수**:
   - 각 씬에 \`dialogue_turns\` 배열을 채우세요. 화자 순서와 의도 명시.
   - 예: \`[{"speaker":"세라핀","intent":"의심 떠보기"},{"speaker":"레온","intent":"추궁"},{"speaker":"세라핀","intent":"부인"}]\`
   - Writer는 이 순서로 화자 태그를 붙입니다. **누락 시 독자가 "누가 말했는지" 알 수 없어 몰입이 깨집니다.**
   - 대사 없는 씬이면 빈 배열 \`[]\`.
   - 턴 수 3~8 이상적. 10턴 초과 시 씬 분할 검토.

14. **회차 내적 시간 (internal_time_span) — 필수**:
   - 이 회차가 소설 내 시간을 얼마나 담는지 반드시 지정.
   - 예: \`{"start":"새벽 5시","end":"정오","duration_hours":7}\`
   - **미지정 시 Writer가 시간을 끝없이 늘려 한 회차에 반나절~하루 과밀 문제 발생.**
   - 일반 회차: 2~8시간. 긴박 장면: 30분~2시간. 시간 점프: 총합을 duration_hours로.

15. **이미 확립된 사실 (already_established) — 서사 루프 방지**:
   - 이전 화들에서 독자가 이미 알게 된 핵심 사실 3~6개를 목록화.
      ...
      "scenes": [
        {
          ...
          "dialogue_turns": [
            {"speaker": "파티원", "intent": "이상함 지적"},
            {"speaker": "이준혁", "intent": "무시하려 함"}
          ]
        },
        {
