...
5. 우연의 일치 금지: 사건의 원인은 반드시 캐릭터의 의도적 행동이어야 합니다.
   - ❌ "종이를 떨어뜨리고, 마침 그 자리에 있던 남주가 주워 읽는다" (우연)
   - ✅ "남주가 성녀의 행동을 수상히 여겨 직접 추궁한다" (의도적 대면)
   - ❌ "마침 지나가던 X가 대화를 엿듣는다" (우연)
   - ✅ "X가 의심을 품고 일부러 몰래 따라간다" (의도)
   - 사건을 연결하는 장치가 "우연히 ~했다"면, 캐릭터의 동기로 바꾸세요.
