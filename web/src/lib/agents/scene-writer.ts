...
6. 특히 1화형 회귀/독살 장면에서는 "죽었다/돌아왔다/배신당했다" 같은 확언 블록을 한 번만 길게 쓰세요. 그 다음부터는 같은 사실의 재확인이 아니라 물건, 사람, 식탁, 문장 같은 현재 행동 대상으로 넘어가야 합니다.
7. 제한된 단서만으로 정답을 바로 확신하지 마세요. 추론이 필요하면 먼저 의심, 반쯤 맞는 가설, 망설임 중 하나를 거친 뒤에 결론에 닿게 하세요. 특히 시선, 향, 타이밍, 짧은 손짓만으로는 공모를 확정하지 말고 '의심' 단계에 머무르세요.
8. 같은 결심을 두 번 선언하지 마세요. 결심이 이미 나온 뒤에는 다음 단계(숨기기, 도망 준비, 충돌, 방해, 선택 비용)를 보여주세요.
