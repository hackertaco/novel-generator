...
export class WriterAgent implements PipelineAgent {
  ...
  async *run(ctx: ChapterContext): AsyncGenerator<LifecycleEvent> {
    const { seed, chapterNumber, previousSummaries, blueprint, previousChapterEnding, fastMode, parallelMode, directionDesign, simpleMode } = ctx;
    ...
    if (blueprint && blueprint.scenes.length > 0) {
      ...
      const sceneResult = await writeFunc({
        ...
        fastMode,
        simpleMode,
        worldStateAuthority: ctx.worldStateAuthority,
      ...
      // Self-review disabled: LLM rewrites undo RuleGuard deterministic fixes
      // and flatten natural prose into AI-style homogeneity.
      // A/B test: fast(0.82) > default(0.78) — self-review was part of the gap.
...
- **1화 엔딩은 절단신공**: 충격적 대사, 위기 순간, 예상 못한 등장 중 하나로 끊으세요. 독자가 "다음 화" 버튼을 누르게 만드세요.
...
2. 짧은 문단 (3문장 이하)
3. 대사 비중 55% 이상
4. **후킹 엔딩 필수** — 마지막 2문단은 반드시 다음 중 하나로 끝내세요:
   - 충격적 대사로 끊기 ("그건 제가 아닙니다." / "…거짓말이었어.")
   - 위기 순간에서 멈추기 (칼이 내려오는 순간, 문이 열리는 순간)
   - 예상 못한 인물/사실 등장 (뒤에 서 있던 사람은~)
   - 말줄임표로 여운 남기기 ("그런데 이 번호는…")
5. 캐릭터 목소리 일관성 유지
6. 장면 묘사, 감정 표현을 풍부하게
...
${rawText.slice(-1500)}`,
    ...
    // Self-review disabled: LLM rewrites undo RuleGuard deterministic fixes.
    // A/B test: fast(0.82) > default(0.78) — self-review contributed to the gap.
    ctx.text = sanitize(rawText);
    yield { type: "replace_text", content: ctx.text };
  }
}
